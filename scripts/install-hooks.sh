...
#   --from-projects         Derive both repo lists from config/projects.yaml instead
#                           of maintaining them by hand. Requires --codebase-root.
#                           Refuses to clobber a hand-edited list; pass --force-derive
#                           to overwrite anyway.
#   --force-derive          Allow --from-projects to overwrite hand-written lists.
...
FROM_PROJECTS=false
FORCE_DERIVE=false
GENERATED_MARKER="# Generated by scripts/derive-hook-repos.py"
GLOBAL=false
WATCH_PATHS=()
WATCH_INDEX_PATHS=()
TRIGGER=""
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --from-projects) FROM_PROJECTS=true; shift ;;
        --force-derive) FORCE_DERIVE=true; shift ;;
...
# 5. Derive the repo lists from projects.yaml
if [ "$FROM_PROJECTS" = true ]; then
    if [ -z "$CODEBASE_ROOT" ]; then
        echo "--from-projects needs --codebase-root" >&2
        exit 1
    fi
    # A list somebody wrote by hand carries intent the YAML does not; never
    # silently discard it.
    for f in "$REPOS_CONFIG" "$INDEX_REPOS_CONFIG"; do
        if [ -s "$f" ] && ! grep -qF "$GENERATED_MARKER" "$f" \
           && grep -qv '^#' "$f" 2>/dev/null && [ "$FORCE_DERIVE" != true ]; then
            echo "Refusing to overwrite hand-written $f — pass --force-derive." >&2
            exit 1
        fi
    done
    PYTHON="$SCRIPT_DIR/../venv/bin/python"
    [ -x "$PYTHON" ] || PYTHON="$(command -v python3)"
    "$PYTHON" "$SCRIPT_DIR/derive-hook-repos.py" \
        --codebase-root "$CODEBASE_ROOT" \
        --config-dir "$MNEMOS_CONFIG_DIR"
fi

# 6. Set trigger if specified
if [ -n "$TRIGGER" ]; then
    echo ""
